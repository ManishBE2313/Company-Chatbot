...
export default function RoleManagementSection({ currentUserEmail, currentUserRole }: RoleManagementSectionProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4 rounded-3xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-indigo-500">Access Control</p>
          <h2 className="mt-1 text-[18px] font-semibold text-slate-800">Role Management</h2>
          <p className="mt-1 text-[13px] text-slate-400">
            Compact card view for assigning roles faster across the organization.
          </p>
        </div>
      ...
      <div className="max-h-[72vh] overflow-y-auto pr-1">
        <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
          {employees.map((employee) => {
            ...
            return (
              <div key={employee.id} className="rounded-3xl border border-slate-200 bg-white px-4 py-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 truncate text-[12px] text-slate-500">{employee.email}</p>
                  </div>

                  <div className="inline-flex shrink-0 items-center gap-1.5 rounded-2xl bg-slate-50 px-2.5 py-1.5 text-[11px] text-slate-500">
                  ...
                  {roles.map((role) => {
                    ...
                    return (
                      <button
                        ...
                        className={selected
                          ? disabled
                            ? "rounded-full border border-indigo-200 bg-indigo-100 px-2.5 py-1 text-[11px] font-semibold text-indigo-500 opacity-70"
                            : "rounded-full border border-indigo-200 bg-indigo-100 px-2.5 py-1 text-[11px] font-semibold text-indigo-700"
                          : disabled
                          ? "rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] text-slate-300"
                          : "rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] text-slate-600 transition-colors hover:border-indigo-200 hover:text-indigo-700"}
                        disabled={disabled}
                      >
                        {role.name}
                      </button>
                ...
                <div className="mt-3 flex items-center justify-between gap-3">
                  <p className="text-[11px] text-slate-400">
                    {saveDisabled ? "Superadmin required for admin access." : employee.department?.name || "No department"}
                  </p>
                  <Button
                    size="sm"
